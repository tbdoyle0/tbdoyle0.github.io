...
unique_lineups <- master %>%
 ...
 # Check time accounted for and team, opponent, and leading scorer total points
...
results_all <- map_dfr(unique_lineups, function(lineup) {
...
results_last <- map_dfr(unique_lineups, function(lineup) {
  # Split lineup into names
  lineup_names <- str_split(lineup, ", ") %>% unlist()
...
results_close <- map_dfr(unique_lineups, function(lineup) {
  # Split lineup into names
  lineup_names <- str_split(lineup, ", ") %>% unlist()
...
results_all <- results_all %>%
    datatable(rownames = FALSE, 
            ...
            callback = JS(
              "function(settings) {",
              "$('head').append('<link href=\"https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&display=swap\" rel=\"stylesheet\">');",
              "$('table').css('font-family', 'Roboto, sans-serif');",  # Apply Roboto font to the table
              "}"
              )
  )
